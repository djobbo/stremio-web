...
const useTranslate = () => {
  ...
  const stringWithPrefix = useCallback(
    (value, prefix, fallback = null) => {
      const key = `${prefix}${value}`
      const defaultValue =
        fallback ?? value.charAt(0).toUpperCase() + value.slice(1)
  ...
  const catalogTitle = useCallback(
    ({ addon, id, name, type } = {}, withType = true) => {
      if (addon && id && name) {
